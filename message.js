...
bot.on(`message`, async ctx => {
    try {
        ...
        if (pola.exec(ctx.message.text)) {
            ...
            pesan += `\n\n🤖 Versi: <code>${process.env['VERSION']}</code>`
            keyb[0] = [
        ...
        if (getSesConMax) {
            ...
            if (Number(text) < 1 || Number(text) > process.env['MAX_CON']) return await bot.telegram.editMessageText(chatID, pros.message_id, null, `⚠️ Hanya angka rentang 1 - ${process.env['MAX_CON']}.`)
        ...
        if (getSesConFile) {
            ...
            var pros = await ctx.reply(`⏳ Memproses...`)
            var text = ctx.message.text
            if (text.length < 1 || text.length > process.env['MAX_CUS_FILE']) return await bot.telegram.editMessageText(chatID, pros.message_id, null, `⚠️ Jumlah karakter pada nama file harus rentang dari 1 - ${process.env['MAX_CUS_FILE']} karakter.`)
            var ops = prop.get(`selection_` + IDs + chatID)
            ...
            if (ops == 'trimVcf') {
                ...
                var maxContacts = getMaxContacts ? Number(getMaxContacts) : process.env['MAX_CON']
